...
from decrypt import check_key_validity
import sys
import configparser
...
combined_message = configs.get('combined_message').data

def check_license_key():
    key_valid = check_key_validity(combined_message.encode('utf-8'))
    if not key_valid:
        root = tk.Tk()
        root.withdraw()  
        messagebox.showerror("License Key Expired", "Your license key has expired. Please contact the admin.")
        sys.exit(1)  
...
#cv2.putText(logoimage, "This is my image", (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 2, (0,255,0), 5)
...
#with open('/var/www/html/ez/python/fr_allconfig/data.txt', 'r') as file:
...
def print_name(name_list,y_ax,imS):
    ...
    for names in name_list:
        ...
        try:
            ...
            img3 = cv2.imread("/var/www/html/ez/python/fr_allconfig/Ez_image/right.jpg")
            img3 = cv2.cvtColor(img3, cv2.COLOR_BGR2RGB)
            img3 = cv2.resize(img3, (550, 550))
            #cv2.imshow('OUTface11', img3)
            x_offset=int(wid/2-img2.shape[1]/2)
            x_offset_2=x_offset+400
            x_offset -= 200
            imS[y_offset:y_offset+img2.shape[0], x_offset:x_offset+img2.shape[1]] = img2
            imS[y_offset:y_offset+img3.shape[0], x_offset_2:x_offset_2+img3.shape[1]] = img3
            cv2.imshow('OUTface', imS)
            cv2.waitKey(1)
...
def clock():
    ...
    while True:
        ...
        bottom_text = data
        bottom_text += " "
        bottom_font = ImageFont.truetype(font_fr, 55)
        text_width, text_height = draw.textsize(bottom_text, font=bottom_font)

        # Split the text into lines that fit the image width
        lines = []
        line = ""
        for word in bottom_text.split():
            if draw.textsize(line + word, font=bottom_font)[0] <= logoimage.shape[1]:
                line += word + " "
            else:
        ...
        if line.strip():
            lines.append(line.strip())
        for line in lines:
            # Calculate text position for left alignment at the bottom
            text_x = 0
            text_y = logoimage.shape[0] - text_height - 10
            draw.rectangle(((0, text_y), (logoimage.shape[1], text_y + text_height)), fill=(100, 149, 237))  
            
            line_with_space = " " + line
            
            draw.text((text_x, text_y), line_with_space, font=bottom_font, fill="#fff")  
            for _ in range(3): 
                n = datetime.now()
...
while(True):
    ...
    if(1 == 2):
        ...
        for detection in results.detections:
          ...
          # os.replace(f'/var/www/html/ez/admin/detected_img/{today}/{face}', f'/var/www/html/ez/python/log_unknownimg/{today}/{face}')
          if(text=="Forward"):
            try: 
