	...
	foreach ($dates as $value)  
						{ 
							$count="1";
								...
								/*header("location: attgeneralday_report.php?fdate=$fdate&tdate=$tdate&rtype=$rtype&rpt=$report_type&shift=$allshift");
							*/
...
(SELECT eid,fname,dep_description,'".$value."' Date,'' Shift,'00:00:00' In_time,'00:00:00' Out_time,'00:00:00' Work_Hours,'00:00:00' OT,'A' `Status`,'' `shift` FROM `emp_details` WHERE `eid` NOT IN (SELECT Employee_ID FROM attendance WHERE date(`Date`)=date('".$value."'))) order by Department ";*/
$search="select a.Employee_ID, a.Name, a.Department, a.Date, a.Shift, a.In_time, a.Out_time, a.Work_Hours, a.OT, a.Status, a.shift from attendance a  where Date='".$value."' UNION
