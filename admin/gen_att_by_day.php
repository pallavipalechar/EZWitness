...
// Retrieve selected date from the form submission or default to today's date
...
// Delete existing data for the selected date range
$delete = "DELETE FROM `attendance` WHERE date(Date)>=date('$fdate') and date(Date)<=date('$tdate')";
mysqli_query($con, $delete);

// Generate attendance records for each date in the selected range
...
foreach ($dates as $value) {
    ...
    $search = "SELECT `eid` FROM `emp_details`";
    $result = mysqli_query($con, $search);
    while ($row = mysqli_fetch_array($result)) {
        $epm_id = $row['eid'];

        // Query to retrieve minimum time for each employee for the selected date
        ...
        while ($row_1 = mysqli_fetch_array($result1)) {
            ...
            // Insert attendance record if not already exists
            ...
            // Query to retrieve maximum time for each employee for the selected date
            ...
            while ($row_2 = mysqli_fetch_array($result2)) {
                ...
                if ($rowco22 == 0) {
                    $search_4 = "UPDATE `attendance` SET Out_time='$Out_time', Work_Hours='$timdif', OT='$OTdif', Status='$Status' where Employee_ID='$em_id' and `Date`='$date'";
                    $result4 = mysqli_query($con, $search_4);
                }
            }
            // Mark the attendance generation as completed for the date
